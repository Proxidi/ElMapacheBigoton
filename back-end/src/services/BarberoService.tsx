...
export default function CRUDBarberoComponent() {
    ...
    const saveBarbero = async () => {
        ...
        if (barbero.nombreBarbero.trim()) {
            ...
            if (barbero.idBarbero) {
                ...
                toast.current?.show({ severity: 'success', summary: 'Éxito', detail: 'Barbero Actualizado', life: 3000 });
            } else {
                _barbero.idBarbero = await getIdBarbero(_barbero);
                _barberos.push(_barbero);
                toast.current?.show({ severity: 'success', summary: 'Éxito', detail: 'Barbero Creado', life: 3000 });
            }
    ...
    const deleteBarbero = () => {
        const _barberos = barberos.filter(b => b.idBarbero !== barbero.idBarbero);
        BarberoService.delete(barbero.idBarbero);
        setBarberos(_barberos);
        setDeleteBarberoDialog(false);
        setBarbero(emptyBarbero);
        toast.current?.show({ severity: 'success', summary: 'Resultado', detail: 'Barbero Eliminado', life: 3000 });
    };

    const findIndexById = (id: number) => barberos.findIndex(b => b.idBarbero === id);

    const exportCSV = () => dt.current?.exportCSV();

    const onNombreBarberoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const val = e.target.value || '';
        setBarbero({ ...barbero, nombreBarbero: val });
    };
    ...
    const deleteBarberoDialogFooter = (
        <React.Fragment>
            ...
            <Button label="Si" icon="pi pi-check" severity="danger" onClick={deleteBarbero} />
        </React.Fragment>
    );
        ...
        <div>
            ...
            <div className="Card">
                ...
                <DataTable ref={dt} value={barberos} dataKey="idBarbero" paginator rows={10} rowsPerPageOptions={[5, 10, 25]}
                    ...
                    <Column field="nombreBarbero" header="Nombre" sortable style={{ minWidth: '20rem' }}></Column>
                    <Column body={actionBodyTemplate} exportable={false} style={{ minWidth: '12rem' }}></Column>
                </DataTable>
            </div>

            <Dialog visible={barberoDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }}
                header="Detalles de Barbero" modal className="p-fluid" footer={barberoDialogFooter} onHide={hideDialog}>
                <div className="field">
                    <label htmlFor="nombreBarbero" className="font-bold">Nombre</label>
                    <InputText id="nombreBarbero" value={barbero.nombreBarbero} onChange={onNombreBarberoChange}
            ...
            <Dialog visible={deleteBarberoDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }}
                ...
                <div className="confirmation-content">
                    ...
                    {barbero && <span>¿Estás seguro de eliminar el barbero <b>{barbero.nombreBarbero}</b>?</span>}
                </div>
            </Dialog>
        </div>
    );
